...
# Caesar Reducer File = sys.argv[2] 
# Sudan Roads Shapefile (.shp needed) = sys.argv[3]
# Date of data accquistion (e.g. 02.02.2025) = sys.argv[4]

# Question Data Minimum Consensus set to 70%
# Bridge Min value of Votes set to 10 people
# Paved Road Min value of Votes set to 7 people
...
def extract_metadata_values(metadata):
    """Extracts coordinate values from metadata string."""
    if pd.isna(metadata):  # Handle missing values
        return None, None, None, None

    try:
        metadata_dict = ast.literal_eval(metadata)  # Convert string to dictionary
...
def bridge_merge_with_external_file(bridge_total_class, updated_subject_file, bridge_merged):
...
if __name__ == "__main__":
    ...
    bridge_filtered = "bridge_filtered.csv"
    ...
    bridge_shapefile = f'Sudan_Priority_Roads_BRIDGES_{date}.shp'
    ...
    subjects_file = sys.argv[1]  # Get file name from command line
    df = pd.read_csv(subjects_file)  # Load CSV file
    ...
    # **Call extract_metadata_values for testing (OPTIONAL)**
    test_metadata = df['metadata'].iloc[0]  # Get first row's metadata
    extracted_values = extract_metadata_values(test_metadata)
    print(f"Extracted values from first row: {extracted_values}")
    ...
    bridge_merge_with_external_file(bridge_total_class, updated_subject_file, bridge_merged)
